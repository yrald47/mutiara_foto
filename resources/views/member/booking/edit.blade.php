            ...
            <div class="card-body">
                <div class="row gx-4 gx-lg-5 align-items-center my-5">
                    ...
                    <div class="col-lg-5">
                        ...
                        <form id="form_validation" method="POST" action="{{ route('booking.store') }}">
                            {{ csrf_field() }}
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" name="kode_booking" value="{{$booking->kode_booking}}" readonly hidden>
                                    <label class="form-label">Waktu Booking</label>
                                    <input type="text" name="kode_paket" value="{{$package->kode_paket}}" hidden readonly>
                                    <div class="input-group">
                                        <input type="date" class="form-control @error('date') is-invalid @enderror" name="date" value="{{$booking->tanggal_take}}" required>
                                        <input type="time" class="form-control @error('time') is-invalid @enderror" name="time" value="{{$booking->jam_take}}" required>
                                      </div>
                                    @error('date')
